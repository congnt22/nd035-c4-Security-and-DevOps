...
public class ControllerTests {
    ...
    public void testFindUserById() {
        User user = createUserManually(1L, "Cong", "hashedPassword");
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        ResponseEntity<User> response = userController.findById(1L);
        Assert.assertNotNull(response);
        Assert.assertEquals(200, response.getStatusCodeValue());
        user = response.getBody();
        Assert.assertEquals("Cong", user.getUsername());
        Assert.assertEquals("hashedPassword", user.getPassword());
    }

    @Test
    public void testFindUserByUsername() {
        User newUser = createUserManually(1L, "Cong", "hashedPassword");
        when(userRepository.findByUsername("Cong")).thenReturn(newUser);
        ResponseEntity<User> response = userController.findByUserName("Cong");
        Assert.assertEquals(200, response.getStatusCodeValue());
    ...
    public void testCreateUser() throws Exception {
        ...
        // Test success case
        ResponseEntity<User> response = createUserUsingController("CongNguyen", "originalPassword", "originalPassword");
        Assert.assertNotNull(response);
        Assert.assertEquals(200, response.getStatusCodeValue());
        User user = response.getBody();
        Assert.assertNotNull(user);
        Assert.assertEquals(0, user.getId());
        Assert.assertEquals("CongNguyen", user.getUsername());
        Assert.assertEquals("hashedPassword", user.getPassword());
        ...
        response = createUserUsingController("CongNguyen", "pass", "pass");
        Assert.assertNotNull(response);
        Assert.assertEquals(400, response.getStatusCodeValue());

        // Test confirm password handler
        response = createUserUsingController("Congn1", "password1", "password2");;
        Assert.assertNotNull(response);
        Assert.assertEquals(400, response.getStatusCodeValue());
    }
    ...
    public void testGetItemById() {
        Item newItem = getNewItem(1L, "Book", new BigDecimal("1.0"), "For studying english");
        ...
        Assert.assertEquals("For studying english", item.getDescription());
    ...
    public void testAddToCart() {
        User user = createUserManually(1L, "Cong", "strongPassword");
        Item item = getNewItem(1L, "Helmet", new BigDecimal(1.0), "best buy");

        when(userRepository.findByUsername("Cong")).thenReturn(user);
        ...
        ResponseEntity<Cart> response = addToCart("Cong", 1L, 1);
        ...
        Assert.assertEquals("Cong", cart.getUser().getUsername());
        ...
        response = addToCart("Not Cong", 1L, 1);
    ...
    public void testRemoveFromCart() {
        ...
        cart = addToCart("Cong", 1L, 5).getBody();
        // Just checking item count
        Assert.assertEquals(5, cart.getItems().size());

        ResponseEntity<Cart> response = removeFromCart("Cong", 1L, 2);
        Assert.assertEquals(200, response.getStatusCodeValue());
        cart = response.getBody();
    ...
    public void testSubmitOrder() {
        ...
        when(userRepository.findByUsername("Cong")).thenReturn(user);
        ResponseEntity<UserOrder> response = orderController.submit("Cong");
        Assert.assertEquals(200, response.getStatusCodeValue());
        ...
        // Test when user is not found
        response = orderController.submit("Not Cong");
        Assert.assertEquals(404, response.getStatusCodeValue());
    }
    ...
    public void testGetOrdersForUser() {
        ...
        ResponseEntity<List<UserOrder>> response = orderController.getOrdersForUser("Cong");
        Assert.assertEquals(200, response.getStatusCodeValue());
        ...
        // Test when user is not found
        response = orderController.getOrdersForUser("Not Cong");
        Assert.assertEquals(404, response.getStatusCodeValue());
    }
